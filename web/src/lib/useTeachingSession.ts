...
function cacheNotebook(notebookId: string, snapshot: NotebookSnapshot): void {
  notebookCache.delete(notebookId);
  notebookCache.set(notebookId, snapshot);
...
export function useTeachingSession(notebookId: string): TeachingSession {
  ...
  const [intake, setIntake] = useState<Intake | null>(() => cached?.intake ?? null);
  const [knowledgeState, setKnowledgeState] = useState<KnowledgeState | null>(() => cached?.knowledgeState ?? null);
