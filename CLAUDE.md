...
Aria is a "reverse tutor": the user teaches, and an AI student (powered by OpenAI Codex via the user's own ChatGPT account) responds with calibrated confusion and probing questions. It also supports interview-practice notebooks where Cyra is the main interviewer. The server spawns `codex app-server` as a child process — the Codex CLI (`npm install -g @openai/codex`) must be on PATH for anything beyond `/api/health` to work.
...
npm test -w @aria/server # focused settings / Codex updater tests
npm test -w @aria/web    # Vitest component and interaction tests
npm run typecheck    # tsc on both workspaces; there is no lint suite
...
Composition root is `server/src/index.ts`; `app.ts` wires routes and a gate middleware that 503s every `/api` route except `/api/health` and the loopback-only `/api/codex/*` maintenance routes unless the app-server is `running`.

**`appserver/`** — `AppServerClient` (client.ts) owns the single long-lived `codex app-server` child: JSON-RPC over stdio (framing in rpc.ts, request/response types in protocol.ts), typed method wrappers (`thread/start`, `turn/start`, …), per-thread notification routing via `subscribeThread`, and crash supervision with backoff (gives up after repeated failures within 60s → state `dead`). Each respawn increments `client.generation`; sessions compare it to know when a Codex thread needs `thread/resume`. A verified local CLI update uses the same controlled restart path, so new binaries take effect without treating the stop as a crash. The student runs in a **read-only sandbox with `approvalPolicy: "never"`** — approval requests should never happen and are auto-declined so a misconfiguration can't hang a turn.
...
- `settings.ts` — global settings (`model`, `effort`, `replyLength`, `probing`, `ragMode`, `ragRecall`). `ARIA_MODEL`/`ARIA_EFFORT` env vars only *seed* the file on first boot; afterwards the file (Settings UI) wins. A null or unavailable model is reconciled to the visible Codex default and persisted explicitly so the checked model is the one sent to every turn.
- `codex-update.ts` — loopback-only Codex CLI maintenance. It detects the exact active npm/Homebrew install, checks the npm registry with a short cached request, runs one bounded fixed update command without shell interpolation or elevation, verifies the binary version changed, then restarts app-server. Unknown/custom paths stay manual.
...
- `discover.ts` / `source-normalize.ts` — online source discovery plus conservative cleanup for navigation debris, opaque media tokens, tables, and TeX. A web-search-enabled Codex turn finds public URLs, then the server downloads/extracts them into individual source files (`origin: "research"`, optional `originUrl`) that are searchable/previewable, deletable, and RAG-indexed. Existing research files are normalized at preview time without being rewritten. Discovery uses `ARIA_RESEARCH_EFFORT` (default `medium`) plus `ARIA_DISCOVER_MAX` (default `5`, clamped 1–10).
...
- `interviewer.ts` / `coverage.ts` — **interview mode** (`type: "interview"` notebooks): no Aria student; the MAIN thread is Cyra as a job interviewer (wire roles unchanged — the human stays `teacher`, the AI stays `student`). Creation collects role/company plus a required CV and optional job description (pasted text becomes ordinary source files flagged `SourceFile.kind: "cv" | "jd"`); the intake form carries format/round/research tuning instead of level/focus (deterministic-only — `generatedQuestions` is preset to `[]`). Interview notebooks never create a `learningState` (the belief evaluator is skipped entirely, including its bootstrap), style drift never restarts the thread (the interviewer persona has no replyLength/probing slots), Ask-Cyra thread creation is rejected, and discovery uses an interview-specific prompt (real interview accounts + company background; forums allowed). `coverage.ts` reuses the `KnowledgeState` shape/SSE/GET plumbing verbatim as an "interview coverage" map — competencies instead of concepts, statuses re-read as not-probed / touched-on / struggled / strong. The end-of-interview debrief is persona-only: the UI sends a canned visible message, no dedicated endpoint.
...
SSE protocol (one channel per notebook, `routes/notebooks.ts` → `lib/sse.ts`): events `state` (snapshot on attach), `turn-started`, `delta`, `message`, structured `activity` (`researching` with search/download progress, kickoff preparation phases, `evaluating-teaching`, `reading-sources`, or `writing-response`), `knowledge-state`, `sources-updated`, `discover-completed`, `notice`, `error`, `turn-completed`, each with an incrementing per-session `id`. The current activity is replayed in `state` after reconnect. Cyra threads have their own `/cyra/:tid/events` channel with the same streaming events minus kickoff/intake/source-discovery fields.
...
- `SettingsDialog` groups autosaving Teaching, Appearance, and Advanced controls; it loads local Codex CLI status only when Advanced opens and offers a verified npm/Homebrew update without blocking other settings.
...
- Icons are a **subset font**: `components/iconNames.ts` is the single source of truth — every icon prop is typed against its `IconName` union (an unlisted name is a typecheck error, not literal ligature text at runtime), and a plugin in `vite.config.ts` injects the list into the Google Fonts `icon_names=` URL in `index.html` at dev/build time (the hardcoded list there is just a no-Vite fallback). To add an icon: add it to `ICON_NAMES`, use it.
