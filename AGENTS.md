...
- `interviewer.ts` / `coverage.ts` — **interview mode** (`type: "interview"` notebooks): no Aria student; the main thread is Cyra as a job interviewer (wire roles remain `teacher` for the human and `student` for the AI). Creation collects role/company plus a required CV and optional job description; intake carries format/round/research tuning. Interview notebooks skip `learningState`, reject Ask-Cyra thread creation, use interview-specific discovery, and reinterpret the persisted `KnowledgeState` as competency coverage (`not probed | touched on | struggled | strong`). The End interview action sends a visible canned request for Cyra's debrief; there is no separate endpoint.
...
- Icons are a **subset font**: `components/iconNames.ts` is the source of truth for typed icon names, and `vite.config.ts` injects that list into the Google Fonts `icon_names=` URL. Add new symbols to `ICON_NAMES` before using them.
