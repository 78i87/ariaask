...
test("recognizes loopback addresses and rejects remote addresses", () => {
  ...
  assert.equal(isLoopbackAddress("::ffff:127.attacker.example"), false);
...
test("local maintenance rejects forwarded and non-local-host requests", () => {
  ...
  assert.equal(isDirectLoopbackRequest(request("::1", "127.attacker.example:5275", undefined, false)), false);
