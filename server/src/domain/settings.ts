...
export interface SelectableModel {
  ...
  supportedReasoningEfforts?: readonly (
    | string
    | { effort?: string; reasoningEffort?: string }
  )[];
}
...
export class SettingsStore {
  ...
  /**
   * The Settings UI always presents one concrete model as selected. Keep the
   * persisted value equally concrete so turns cannot silently inherit a
   * different model from the user's global Codex config.
   */
  async reconcileModel(models: readonly SelectableModel[]): Promise<Settings> {
    if (models.length === 0) return this.settings;
    const selected =
      (this.settings.model ? models.find((m) => m.model === this.settings.model) : null) ??
