...
export interface IntakeAnswers {
  ...
  skipped: boolean;
  /** Interview notebooks only: chosen option value or custom text. Absent/null elsewhere. */
...
// Static, never model-generated (interview notebooks preset generatedQuestions
// to [] at creation, which also locks out the teach-framed generator).
...
export const INTERVIEW_RESEARCH_QUESTION: IntakeQuestion = {
  // Reuses the "research" id so the answer flows through the same typed
  // research mapping as teach mode.
  id: "research",
  question: "May Cyra research the company and real interview experiences online first?",
  options: [
    { value: "yes", label: "Yes — let Cyra find interview accounts and company background" },
...
export function composeIntakeQuestions(nb: Notebook): IntakeQuestion[] {
  if (nb.type === "interview") {
    // Research is always offered: company/role research is valuable even
    // though a CV is always present (unlike teach mode's sources condition).
