...
import type { AppSettings, CoachMode, CodexCliStatus, ModelInfo } from "../../lib/types";
...
export function SettingsDialog({ open, onClose }: SettingsDialogProps) {
  ...
  const load = useCallback(async () => {
    // Reopening Settings refreshes in the background. Keep the last valid
    // snapshot visible so a slow model/usage request cannot replace the whole
    // dialog with an indefinite spinner.
    ...
    try {
      const [res, usage] = await Promise.all([
        api.getSettings(),
        Promise.resolve(api.getUsage()).catch(() => null),
      ]);
      setSettings(res.settings);
      setModels(res.models);
      if (usage?.usage) setCoachMode(usage.usage.coachMode);
  ...
  const updateCoachMode = useCallback(
    (mode: CoachMode) => {
      ...
      setCoachMode(mode);
      setSaveState("saving");
      void api.updateUsage({ coachMode: mode }).then(
        () => {
          setSaveState("saved");
        ...
        <div className="settings__sections">
          ...
          {section === "teaching" && <section className="settings__section">
            ...
            <Segmented
              ...
              value={coachMode}
              onChange={(value) => updateCoachMode(value as CoachMode)}
            />
            <span className="settings__supporting body-medium">
              {coachMode === "guided"
                ? "Full scaffolding — the coach explains which technique fits and how to run it."
                : coachMode === "intermediate"
                  ? "Lighter scaffolding — the coach names the move and the mistake to avoid."
                  : "Peer mode — you bring the plan and the coach critiques it."}
            </span>
          </section>}
